...
### ✅ Correct Pattern: Inline Entity Creation

**Solution**: Create all entities in the same context
...
// 2. Create venue with FK to tenant we just created
var venue = new Venue
{
    TenantId = tenant.Id,  // Valid FK - same context
...
// 3. Create act with FK to same tenant
var act = new Act
{
    TenantId = tenant.Id,  // Valid FK - same context
...
Always use `tenant.Id` (the database PK) for foreign key relationships:
...
// ✅ Use database ID for FK
var venue = new Venue 
{ 
    TenantId = tenant.Id,  // e.g., 1 (auto-generated)
    Name = "Venue" 
};
...
{ 
    TenantId = testTenantId,  // WRONG! This is 4523, not a valid tenant ID
    Name = "Bad Venue"
};
```
...
using (var setupContext = _fixture.CreateDbContext(_connectionString, null))
{
    ...
    var venue = new Venue { TenantId = tenant.Id, ... };
    setupContext.Venues.Add(venue);
    await setupContext.SaveChangesAsync();
}
...
public async Task GetShow_FromOtherTenant_Returns404()
{
    ...
    using (var contextA = _fixture.CreateDbContext(_connectionString, null))
    {
        ...
        var venue = new Venue { TenantId = tenantA.Id, ... };
        contextA.Venues.Add(venue);
        var act = new Act { TenantId = tenantA.Id, ... };
...
- [ ] Use `tenant.Id` (database PK) for foreign keys, not test isolation IDs
- [ ] Dispose contexts properly with `using` statements
- [ ] Apply `lock` around `Database.EnsureCreated()` for thread safety
